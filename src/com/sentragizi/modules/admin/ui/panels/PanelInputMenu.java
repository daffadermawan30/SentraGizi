...
public class PanelInputMenu extends JPanel {
    ...
    private void actionAdd(ActionEvent e) {
        ...
        if (selectedObj != null) comp = selectedObj.toString().trim();
    ...
    private void actionReset(ActionEvent e) { resetForm(); }
    ...
    private void actionShowList(ActionEvent e) {
        ...
        DefaultTableModel listModel = new DefaultTableModel(new String[]{"ID", "Nama Menu"}, 0) {
            @Override public boolean isCellEditable(int row, int col) { return false; }
        };
        List<Menu> menus = repo.getAllMenus();
        for (Menu m : menus) listModel.addRow(new Object[]{m.getId(), m.getName()});

        JTable listTable = new JTable(listModel);
        listTable.setRowHeight(30);
        listTable.getColumnModel().getColumn(0).setMaxWidth(50); 
        listTable.setFont(new Font("Segoe UI", Font.PLAIN, 13));
            ...
            if (row != -1) {
                ...
                if (confirm == JOptionPane.YES_OPTION) {
                    if (repo.deleteMenu(id)) {
                        ...
                        if (selectedMenuId == id) resetForm();
